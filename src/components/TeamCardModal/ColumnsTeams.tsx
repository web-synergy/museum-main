  ...
  return (
    <Box key={columnIndices[0]} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '40px' }}>
      {columnIndices.map((index: number) => (
        <Box key={index} sx={{ width: '100%', textAlign: { xs: 'center', md: 'left' } }}>
          <Typography variant="h6" sx={{ fontSize: '18px', fontWeight: '600', lineHeight: 'normal', color: theme.palette.primary.main }}>
